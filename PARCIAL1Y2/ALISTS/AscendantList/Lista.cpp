...
void Lista::InsertarOrden(int nuevo)
{
	...
	if (!cabecera)
	{
		...
		cabecera = nodo;
		fin = nodo;
		cout << "\tOK. Se insert� " << nuevo << " al inicio " << endl;
		return;
	}
	anterior = NULL;
	actual = cabecera;
	...
	if (!encontrado)
	{
		...
		fin->siguiente = nodo;
		fin = nodo;
		cout << "\tOK. Se insert� " << nuevo << " al final " << endl;
		return;
	}
	if (!anterior)
	{
	...
	else
	{
		...
		cout << "\tOK. Se insert� " << nuevo << " despu�s de " << anterior->num << endl;
	}
...
int Lista::ExtraerInter(int numExtraer)
{
	...
	if (!bandera)	// Cas� en que no existe en la lista el n�mero buscado
	{
	...
	if (nodo == fin)	// Caso en que el nodo extraido es el �ltimo de la lista
		fin = anterior;
